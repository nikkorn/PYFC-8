...
public class TerminalArea {
     ...
     */
    private ArrayList<com.dumbpug.sfc.components.terminal.Line> lines = new ArrayList<com.dumbpug.sfc.components.terminal.Line>();
    /**
     * The input line.
     */
    private com.dumbpug.sfc.components.terminal.Line inputLine;
    /**
    ...
    public TerminalArea(float x, float y, float lineHeight, float columnWidth, int lines, int columns, TerminalAreaConfiguration config) {
        ...
        this.inputLine = new com.dumbpug.sfc.components.terminal.Line(config.inputPrompt);
     ...
     */
    public void moveCursor(com.dumbpug.sfc.components.terminal.CursorMovement movement) {
        switch (movement) {
            case LEFT:
    ...
    public void insertText(String text, Colour colour) {
        ...
        for (int characterIndex = 0; characterIndex < text.length(); characterIndex++){
            ...
            this.moveCursor(com.dumbpug.sfc.components.terminal.CursorMovement.RIGHT);
        }
    }
    ...
    private void drawText(SpriteBatch batch, BitmapFont font) {
        ...
        for (int lineIndex = lineOffset + lineCount - 1; lineIndex >= lineOffset; lineIndex--) {
            ...
            com.dumbpug.sfc.components.terminal.Line line = this.lines.get(lineIndex);

            // Get the columns that make up the current line.
            ArrayList<com.dumbpug.sfc.components.terminal.Column> columns = line.getColumns();
    ...
    private void printOutputText(String text, Colour colour) {
        if (this.printToNewOutputLine || this.lines.size() == 1) {
            this.lines.add(this.lines.size() - 1, new com.dumbpug.sfc.components.terminal.Line());
        }

        // Get the last output line to print to.
        com.dumbpug.sfc.components.terminal.Line outputLine = this.getLastOutputLine();
